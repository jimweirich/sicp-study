(test-case "Testing Expected Messages"
           ...
           (assert-equal "Expected <1> but got <2>" (expected-equal-message 1 2))
           (assert-equal "Expected <1> to not equal <1>" (expected-not-equal-message 1 1))
           (assert-equal "Expected the delta of <1.> and <2.> to be less than <.5> but was <1.>"
                         (expected-in-delta-message 1.0 2.0 0.5))
           (assert-equal "Expected the delta of <1.> and <2.> to be greater than <2.> but was <1.>"
                         (expected-not-in-delta-message 1.0 2.0 2.0)))
